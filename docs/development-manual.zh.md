...
> **功能以最小模組進行開發。**
>
> - **核心目標：** 實現「高內聚、低耦合」，確保系統易於測試與替換。
> - **手段：** 將大需求拆解為「原子級」的模組（Atomic Modules），每個模組只做一件事情。

本手冊其餘章節皆為此原則的具體展開：

- [§3 原子模組目錄](#第-3-部分--原子模組目錄) — 600 個原子盤點與 19 種模式
- [§5.1 原子化程式的 8 條規則](#5-1-原子化程式的-8-條規則) — 怎麼寫一個原子
- [§9.1 三層架構](#9-1-三層架構) — 介面 / 服務 / 基礎設施分層
- [§10.6 標準目錄結構](#10-6-標準目錄結構功能導向遞迴模組化) — 用 `features/<name>/` 包裝原子的標準佈局
...
| 1.4 | 2026-04-29 | 新增「開發原則」前言：將「功能以最小模組進行開發」作為宣言放在 §1 之前，連結到 §3／§5.1／§9.1／§10.6 的具體展開 |
